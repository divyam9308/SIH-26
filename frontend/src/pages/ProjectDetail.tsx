...
export function ProjectDetail() {
  ...
  return <div className="projects-page project-detail">
    ...
    <section className="prediction-summary" aria-label="Executive prediction summary">
      ...
      <article className="prediction-card"><p>Predicted Time Overrun</p><strong className={tone}>{forecast ? `${forecast.predicted_delay_months.toFixed(1)} months` : 'Unavailable'}</strong><span>{forecast ? `${forecast.predicted_delay_days.toFixed(0)} total days beyond the original schedule` : unavailable}</span><small>{forecast?.predicted_completion_date ? `Predicted completion ${formatDate(forecast.predicted_completion_date)}` : 'MAE measures average error in days, not the size of this project’s overrun.'}</small></article>
    ...
    <div className="detail-grid"><ProjectPanel title="Timeline Intelligence" className="span-two" action={<Clock size={16} color="var(--p-muted-foreground)" />}><dl className="detail-fields"><Field label="Project start" value={unavailable} /><Field label="Original completion" value={formatDate(project.original_end_date)} /><Field label="Latest revised completion" value={formatDate(project.revised_end_date)} /><Field label="Predicted completion" value={forecast ? formatDate(forecast.predicted_completion_date) : 'Unavailable'} accent={forecast ? tone : ''} /><Field label="Predicted total time overrun" value={forecast ? `${forecast.predicted_delay_months.toFixed(1)} months` : 'Unavailable'} accent={forecast ? tone : ''} /></dl>{forecast && !forecast.predicted_completion_date && <p className="range-note">This is the total predicted delay beyond the original schedule. A lower delay MAE means the model is closer to recorded delays on average; it does not mean each project’s total delay is smaller.</p>}{forecast?.expected_range && <p className="range-note">Expected delay range: {forecast.expected_range.delay_days.p10.toFixed(1)} to {forecast.expected_range.delay_days.p90.toFixed(1)} days.</p>}</ProjectPanel>
