...
function ScatterPanel({ title, rows, kind }: { title: string; rows: ValidationRow[] | null; kind: 'cost' | 'delay' }) {
  ...
  const formatAxisTick = (value: number) => Math.round(value).toLocaleString();
  ...
  return <Panel title={title} icon={cost ? <TrendingUp /> : <Clock3 />} className="pa-chart-panel" action={<div className="pa-legend"><span><Mark color={blue} dash />Perfect prediction</span><span><Mark color={pale} />±{tolerance} {cost ? 'pp' : 'days'} band</span></div>}>
    {!rows ? <Empty>Validation rows are not published for the 2001–2021 production retrain.</Empty> : !points.length ? <Empty>No plottable validation rows were returned.</Empty> : <div className="pa-chart"><ResponsiveContainer><ScatterChart margin={{ top: 12, right: 10, bottom: 22, left: 5 }}><CartesianGrid stroke="#e3eaf3" /><XAxis dataKey="actual" type="number" domain={[low, high]} ticks={axisTicks} tickFormatter={formatAxisTick} tick={{ fontSize: 9 }} /><YAxis dataKey="predicted" type="number" domain={[low, high]} ticks={axisTicks} tickFormatter={formatAxisTick} width={48} tick={{ fontSize: 9 }} /><ReferenceLine segment={[{ x: low, y: low }, { x: high, y: high }]} stroke={blue} strokeDasharray="5 4" /><Tooltip content={<ScatterTip />} /><Scatter data={points} fill={cost ? blue : orange} fillOpacity={0.46} stroke="none" /></ScatterChart></ResponsiveContainer></div>}
  </Panel>;
}
...
export function PredictionAccuracyPage() {
  ...
  return <div className="prediction-accuracy-page"><div className="pa-product-header"><div><b>PAIMANA</b><span>MoSPI · Project Risk Intelligence</span></div><div><em className="production"><i />Production model</em><em>SIH 26103</em></div></div><main className="pa-content">
    <div className="pa-heading"><div><h1>Prediction Accuracy</h1><p>Official production retrain · 2001–2021 training</p></div><span className="verified"><CheckCircle2 />Verified production evidence</span></div>
    {error ? <section className="pa-error"><AlertTriangle /><span>{error}</span><button onClick={refresh}>Retry</button></section> : <>
      ...
      <section className="pa-kpis"><Kpi icon={<BarChart3 />} tone="blue" title="Cost Forecast" result={metric(report?.cost_model.MAE, 'pp')} footer="MAE on published evaluation" /><Kpi icon={<Clock3 />} tone="orange" title="Delay Forecast" result={metric(report?.delay_model.MAE_days ?? report?.delay_model.MAE, 'days', 2)} footer="MAE on published evaluation" /><Kpi icon={<AlertTriangle />} tone="red" title="Risk Classification" result={num(risk?.macro_f1 ?? risk?.f1, 4)} footer="Macro-F1" /><Kpi icon={<ShieldCheck />} tone="green" title="Validation Status" result="Published" footer="Held-out outcomes excluded from fitting" /></section>
