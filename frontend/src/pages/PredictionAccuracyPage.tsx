...
function ScatterPanel({ title, rows, kind }: { title: string; rows: ValidationRow[] | null; kind: 'cost' | 'delay' }) {
  ...
  const points = (rows ?? []).flatMap((row) => {
    ...
    return valid(actual) && valid(predicted) ? [{ actual, predicted, name: row.project_name ?? row.project_id ?? 'Project' }] : [];
  });
  const axis = points.flatMap((point) => [point.actual, point.predicted]);
  ...
  return <Panel title={title} icon={cost ? <TrendingUp /> : <Clock3 />} className="pa-chart-panel" action={<div className="pa-legend"><span><Mark color={blue} dash />Perfect prediction</span><span><Mark color={pale} />±{tolerance} {cost ? 'pp' : 'days'} band</span></div>}>
    {!rows ? <Empty>Validation rows are not published for the 2001–2021 production retrain.</Empty> : !points.length ? <Empty>No plottable validation rows were returned.</Empty> : <div className="pa-chart"><ResponsiveContainer><ScatterChart margin={{ top: 12, right: 10, bottom: 22, left: 5 }}><CartesianGrid stroke="#e3eaf3" /><XAxis dataKey="actual" type="number" domain={[low, high]} tick={{ fontSize: 9 }} /><YAxis dataKey="predicted" type="number" domain={[low, high]} tick={{ fontSize: 9 }} /><ReferenceLine segment={[{ x: low, y: low }, { x: high, y: high }]} stroke={blue} strokeDasharray="5 4" /><Tooltip content={<ScatterTip />} /><Scatter data={points} fill={cost ? blue : orange} fillOpacity={0.46} stroke="none" /></ScatterChart></ResponsiveContainer></div>}
  </Panel>;
}
...
export function PredictionAccuracyPage() {
  ...
  const facts: Array<[typeof CalendarDays, string, string]> = [
    [CalendarDays, 'Train', `${metadata?.training_start ?? 2001} – ${metadata?.training_end ?? 2021}`],
    [CalendarDays, 'Future Test', `${metadata?.test_start ?? 2022} – ${metadata?.test_end ?? 2025}`],
    [Layers3, 'Training Projects', count(metadata?.training_projects ?? metadata?.unique_training_projects)],
    [Layers3, 'Test Projects', count(metadata?.evaluation_projects ?? metadata?.unique_test_projects ?? data?.total)],
    [ClipboardList, 'Training Snapshots', count(metadata?.training_snapshots)],
    [ClipboardList, 'Test Snapshots', count(metadata?.test_snapshots)],
    [BarChart3, 'Feature Quality', valid(metadata?.feature_quality?.data_quality_score) ? `${num(metadata.feature_quality.data_quality_score, 2)}%` : 'Unavailable'],
  ];
  ...
  return <div className="prediction-accuracy-page"><div className="pa-product-header"><div><b>PAIMANA</b><span>MoSPI · Project Risk Intelligence</span></div><div><em className="production"><i />Production model</em><em>SIH 26103</em></div></div><main className="pa-content">
    <div className="pa-heading"><div><h1>Prediction Accuracy</h1><p>Official production retrain · 2001–2021 training · 2022–2025 future holdout</p></div><span className="verified"><CheckCircle2 />Verified production evidence</span></div>
    {error ? <section className="pa-error"><AlertTriangle /><span>{error}</span><button onClick={refresh}>Retry</button></section> : <>
      <section className={`pa-evidence ${loading ? 'loading' : ''}`}>{facts.map(([Icon, label, result], index) => <div key={label}><Icon /><span>{label}</span><b>{result}</b>{index < facts.length - 1 && <i />}</div>)}</section>
      <section className="pa-kpis"><Kpi icon={<BarChart3 />} tone="blue" title="Cost Forecast" result={metric(report?.cost_model.MAE, 'pp')} footer="MAE on 2022–2025 holdout" /><Kpi icon={<Clock3 />} tone="orange" title="Delay Forecast" result={metric(report?.delay_model.MAE_days ?? report?.delay_model.MAE, 'days', 2)} footer="MAE on 2022–2025 holdout" /><Kpi icon={<AlertTriangle />} tone="red" title="Risk Classification" result={num(risk?.macro_f1 ?? risk?.f1, 4)} footer="Macro-F1 on 2022–2025 holdout" /><Kpi icon={<ShieldCheck />} tone="green" title="Validation Status" result="Future Holdout" footer="No 2022–2025 outcomes used in fitting" /></section>
      ...
      <section className="pa-lower"><Panel title="Regression Metrics" icon={<BarChart3 />}><div className="regression"><div className="model cost"><h3>Cost model</h3><p><span>MAE</span><b>{metric(report?.cost_model.MAE, 'pp')}</b></p><p><span>R²</span><b>{num(report?.cost_model.R2, 4)}</b></p><p><span>MAPE</span><b>{metric(report?.cost_model.MAPE, '%')}</b></p></div><div className="model delay"><h3>Delay model</h3><p><span>MAE</span><b>{metric(report?.delay_model.MAE_days ?? report?.delay_model.MAE, 'd', 2)}</b></p><p><span>R²</span><b>{num(report?.delay_model.R2, 4)}</b></p><p><span>MAPE</span><b>{metric(report?.delay_model.MAPE, '%')}</b></p></div></div></Panel><Panel title="Classification Metrics" icon={<TrendingUp />}><div className="classification"><MiniMetric label="Precision" result={num(risk?.macro_precision ?? risk?.precision, 4)} tone="blue" /><MiniMetric label="Recall" result={num(risk?.macro_recall ?? risk?.recall, 4)} tone="orange" /><MiniMetric label="F1" result={num(risk?.macro_f1 ?? risk?.f1, 4)} tone="green" /></div><p className="panel-note">Risk classification on 2022–2025 temporal holdout</p></Panel><Panel title="Performance Across Time" icon={<Clock3 />}>{!data?.rolling?.folds?.length ? <Empty>Annual 2022–2025 holdout metrics are unavailable.</Empty> : <div className="pa-chart"><ResponsiveContainer><LineChart data={data.rolling.folds} margin={{ top: 12, right: 14, bottom: 8, left: 4 }}><CartesianGrid stroke="#e3eaf3" /><XAxis dataKey="test_year" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} /><Tooltip /><Legend iconType="circle" iconSize={7} wrapperStyle={{ fontSize: 10 }} /><Line dataKey="cost_MAE" name="Cost MAE (pp)" stroke={blue} strokeWidth={2} dot={{ r: 3 }} /><Line dataKey="delay_MAE_days" name="Delay MAE (days)" stroke={orange} strokeWidth={2} dot={{ r: 3 }} /></LineChart></ResponsiveContainer></div>}</Panel></section>
      <Panel title="Project-level Validation Sample" icon={<ClipboardList />} className="sample" action={<button className="more" onClick={() => setMore((value) => !value)}>{more ? 'Show fewer samples' : 'View more samples'} →</button>}>{!data?.rows?.length ? <Empty>Project-level 2022–2025 validation evidence is unavailable.</Empty> : <div className="table-wrap"><table><thead><tr>{['Project ID', 'Predicted Cost (%)', 'Actual Cost (%)', 'Cost Error (pp)', 'Predicted Delay (days)', 'Actual Delay (days)', 'Delay Error (days)', 'Confidence'].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{data.rows.slice(0, more ? 20 : 5).map((row, index) => <tr key={`${row.project_id}-${index}`}><td>{row.project_id ?? 'Unavailable'}</td><td>{num(row.predicted_cost_overrun, 1)}</td><td>{num(row.actual_cost_overrun, 1)}</td><td>{num(row.cost_error, 1)}</td><td>{num(row.predicted_delay_days, 0)}</td><td>{num(row.actual_delay_days, 0)}</td><td>{num(row.delay_error, 0)}</td><td>{valid(row.model_confidence_percentage) ? `${num(row.model_confidence_percentage, 1)}%` : 'Unavailable'}</td></tr>)}</tbody></table></div>}</Panel>
      <p className="footer-note">Every plotted value comes from the fixed 2001–2021 production retrain evaluated only on projects completed in 2022–2025.</p>
    </>}
  </main></div>;
}
