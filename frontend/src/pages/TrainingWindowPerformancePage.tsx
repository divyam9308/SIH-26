...
function ErrorTrendCard({ windows }: { windows: TrainingWindowMetric[] }) {
  const valueLabel = ({ index, value, x, y }: { index?: number; value?: unknown; x?: string | number; y?: string | number }) => {
    if (typeof value !== 'number' || x === undefined || y === undefined) return null;
    const isFirst = index === 0;
    return <text x={Number(x) + (isFirst ? 10 : 0)} y={Number(y) - 9} textAnchor={isFirst ? 'start' : 'middle'} fill="#1e293b" fontSize={11} fontWeight={800}>{value.toFixed(3)}</text>;
  };
  const chart = (key: 'cost_mae' | 'delay_mae_days', label: string, color: string, suffix: string) => <div className="twp-trend" key={key}><div className="twp-trend-title"><h3>{label}</h3><span><i style={{ backgroundColor: color }} />{suffix}</span></div><div className="twp-chart"><ResponsiveContainer><LineChart data={windows} margin={{ top: 25, right: 18, left: 0, bottom: 0 }}><CartesianGrid stroke="#e4eaf2" /><XAxis dataKey="end_year" tick={{ fontSize: 12, fill: '#64748b' }} axisLine={{ stroke: '#cbd5e1' }} tickLine={false} /><YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} width={56} domain={['auto', 'auto']} /><Tooltip formatter={(value) => [typeof value === 'number' ? `${value.toFixed(3)}${key === 'cost_mae' ? '%' : ' days'}` : 'Unavailable', suffix]} labelFormatter={(year) => `Training window ending ${String(year)}`} /><Line type="monotone" dataKey={key} name={suffix} stroke={color} strokeWidth={3} dot={{ r: 5, fill: color, stroke: '#fff', strokeWidth: 2 }} activeDot={{ r: 6 }}><LabelList dataKey={key} content={valueLabel} /></Line></LineChart></ResponsiveContainer></div></div>;
