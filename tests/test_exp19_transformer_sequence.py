...
from backend.app.ml.experiments.transformer_sequence_exp19 import TransformerForecaster


def _history(months: int = 72) -> pd.DataFrame:
    dates = pd.date_range("2016-01-31", periods=months, freq="ME")
    return pd.DataFrame([{"canonical_project_id":"P1","snapshot_date":stamp,"approved_cost_cr":100.0,"revised_cost_cr":100.0+max(0,i-20)*0.8,"cumulative_expenditure_cr":2.0+i*1.1,"physical_progress":min(100.0,i*1.4),"schedule_slippage_days":max(0.0,(i-24)*4.0),"planned_duration_days":1095.0,"expected_progress_percentage":min(100.0,i*1.5)} for i,stamp in enumerate(dates)])


def test_sequence_store_is_future_append_invariant():
    history=_history(24); cutoff=history.iloc[11].snapshot_date; before=SequenceStore(history).raw("P1",cutoff,None)
...
def test_transformer_forward_supports_variable_lengths():
    width=len(SEQUENCE_FEATURES)*2; model=TransformerForecaster(width,[4,5,3])
    out=model(torch.zeros((2,20,width)),torch.tensor([20,7]),torch.zeros((2,8)),torch.zeros((2,3),dtype=torch.long))
    assert tuple(out.shape)==(2,2)


def test_exp19_is_registered_as_default_challenger():
    adapter=get_experiment_adapter("exp_19"); assert adapter.sequence==19 and adapter.scope=="cost_delay"
    assert default_experiment_adapter().experiment_id=="exp_19"
