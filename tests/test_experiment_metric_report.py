...
def test_delay_report_accepts_real_r2_improvement_without_material_mae_regression():
    report = build_report(baseline=payload(), candidate=payload(delay_r2=.84, delay_mae=343.0), target="delay_r2")
    assert report["decision"] == "ACCEPT"
    assert report["delta_candidate_minus_baseline"]["delay_r2"] > 0


def test_delay_report_rejects_no_change():
    assert build_report(baseline=payload(), candidate=payload(), target="delay_r2")["decision"] == "REJECT"
