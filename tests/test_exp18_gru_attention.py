...
from backend.app.ml.experiments.gru_attention_exp18 import GRUAttentionForecaster
...
def test_gru_attention_forward_supports_variable_lengths():
    width=len(SEQUENCE_FEATURES)*2; model=GRUAttentionForecaster(width,[4,5,3])
    out=model(torch.zeros((2,20,width)),torch.tensor([20,7]),torch.zeros((2,8)),torch.zeros((2,3),dtype=torch.long))
    assert tuple(out.shape)==(2,2)


def test_exp18_is_registered_as_default_challenger():
    adapter=get_experiment_adapter("exp_18"); assert adapter.sequence==18 and adapter.scope=="cost_delay"
    assert default_experiment_adapter().experiment_id=="exp_18"
