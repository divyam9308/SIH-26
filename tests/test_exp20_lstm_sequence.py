...
from backend.app.ml.experiments.lstm_sequence_exp20 import LSTMForecaster
...
def test_lstm_forward_supports_variable_lengths():
    width=len(SEQUENCE_FEATURES)*2; model=LSTMForecaster(width,[4,5,3])
    out=model(torch.zeros((2,20,width)),torch.tensor([20,7]),torch.zeros((2,8)),torch.zeros((2,3),dtype=torch.long))
    assert tuple(out.shape)==(2,2)


def test_exp20_is_registered_as_default_challenger():
    adapter=get_experiment_adapter("exp_20"); assert adapter.sequence==20 and adapter.scope=="cost_delay"
    assert default_experiment_adapter().experiment_id=="exp_20"
