...
def fast_current_production(data: pd.DataFrame, start: int, end: int, test_end: int) -> tuple[dict, dict]:
    ...
    cost_name, _discarded_cost, cost_cmp = _select_regressor(
        train, production_features, "actual_cost_overrun_percentage", 26203
    )
    delay_name, delay_model, delay_cmp = _select_regressor(
