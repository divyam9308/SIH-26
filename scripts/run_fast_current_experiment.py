"""Fast exact Cost/Delay production-vs-experiment audit.

Reproduces current production's feature audit, temporal model-family selection,
promoted Exp12 Cost trajectory selection, seeds, weighting and holdout while
skipping risk fitting, SHAP, ablations and artifact publication.
"""
...
def fast_current_production(data: pd.DataFrame, start: int, end: int, test_end: int) -> tuple[dict, dict]:
    ...
    cost_model = _fit_pipeline(
        _regressors(PRODUCTION_COST_SEED)[cost_name], enriched_train, cost_features, "actual_cost_overrun_percentage"
    )
    ...
    metadata = {
        ...
        "internal_cost_trajectory_feature_comparisons": cost_feature_cmp,
    }
    bundle = {"metadata": metadata, "cost": cost_model, "delay": delay_model}
    receipt = {
        "run_id": f"fast-current-production-{start}-{end}",
        "selected_algorithms": selected,
        "features_used": production_features,
        "production_cost_baseline": PRODUCTION_COST_BASELINE,
...
def main() -> None:
    ...
    payload = {
        ...
        "production": receipt, "production_metadata": bundle["metadata"],
        "experiment": experiment, "overall_comparison": overall,
    }
    ...
    summary = {
        ...
        "cost_improvement_percentage": overall.get("cost_improvement_percentage"),
