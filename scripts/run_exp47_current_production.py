"""Freshly retrain exact production, then evaluate isolated Experiment 47."""
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="exp47-production-") as temp:
        ...
        fitted = get_experiment_adapter("exp_47").module.fit_against_production(
            data=data,
    ...
    if before != _production_hashes():
        raise AssertionError("Exp47 modified a production artifact")
    ...
    if args.start == 2001 and args.end == 2021 and args.test_end == 2025:
        ...
        if not valid:
            print("EXP47_EXECUTION_VERDICT=EXECUTION INVALID")
            raise RuntimeError(f"Exp47 freshly reproduced baseline violates contract: {observed}")

    payload = {
        "window": f"{args.start}_{args.end}",
        "test_end": args.test_end,
        "baseline_mode": "fresh exact production Exp35 retraining in isolated temporary artifacts",
    ...
    prefix = f"EXP47_{args.start}_{args.end}"
    ...
    print(f"EXP47_AUX_REVISION_EVENTS={fitted['experiment']['auxiliary_diagnostics']['observed_revision_events']}")
    print("EXP47_EXECUTION_VERDICT=EXECUTION VALID")
    print(f"EXP47_SCIENTIFIC_VERDICT={overall['scientific_verdict']}")
