"""Run Exp33 residual calibration against the exact current Exp34 Delay stack.

This scientific harness reproduces Exp12 Cost and Exp34 Delay training while
skipping risk fitting, SHAP and artifact publication, none of which can affect
Cost/Delay MAE.
"""
...
def exact_current_cost_delay_production(
    ...
    bundle["delay"] = ProductionDelayBlendModel(delay_models, delay_weights, delay_features)
...
def main() -> None:
    ...
    bundle, receipt = exact_current_cost_delay_production(data, args.start, args.end, args.test_end)
    adapter = get_experiment_adapter("exp_33_on_exp34")
    ...
    payload = {
        ...
        "test_end": args.test_end,
        "audit_mode": "exact current Exp12 Cost + Exp34 Delay; Exp33 Delay calibration only",
        "production": receipt,
    ...
    print(
        "EXP33_ON_EXP34_COMPARISON="
        + json.dumps(payload["overall_comparison"], sort_keys=True, default=str, allow_nan=False)
    )
