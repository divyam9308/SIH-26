#!/usr/bin/env python3
"""Fast controlled production-vs-Exp16 GRU comparison.

This runner reconstructs the current production cost/delay paths without the
risk/SHAP/publication overhead of the full production trainer, then delegates to
the isolated Exp16 adapter.  The future holdout is never used for neural history
length selection.
"""
...
from backend.app.ml.experiments.adapter_exp16 import fit_against_production
...
def reproduce_production(data: pd.DataFrame, start: int, end: int, test_end: int):
    raw = data.copy()
    raw["completion_year"] = pd.to_numeric(raw.completion_year, errors="coerce")
    ...
    cost_name, _base_cost, cost_selection = _select_regressor(
        raw_train, production_features, "actual_cost_overrun_percentage", 26203
    )
    delay_name, delay_model, delay_selection = _select_regressor(
        raw_train, production_features, "actual_delay_days", 26204
    )

    enriched = enrich_supervised_for_production(raw.copy())
    ...
    cost_added, cost_group, cost_feature_selection = select_exp12_features(
        train,
        production_features,
        usable,
    ...
    metadata = {
        ...
        "delay_features_used": production_features,
        "risk_features_used": production_features,
    ...
    receipt = {
        "run_id": f"exp16-controlled-{start}-{end}",
        "selected_algorithms": {"cost": cost_name, "delay": delay_name},
    }
    reproduction = {
        "production_cost_baseline": PRODUCTION_COST_BASELINE,
        ...
        "ordinary_algorithm_selection": {"cost": cost_selection, "delay": delay_selection},
        "cost_trajectory_selection": cost_feature_selection,
    }
...
def main() -> int:
    ...
    test_end = int(args.test_end or latest)
    if test_end <= args.end:
        raise ValueError(f"test_end={test_end} must be after training_end={args.end}")

    production_bundle, production_receipt, reproduction = reproduce_production(
        data, args.start, args.end, test_end
    )
    result = fit_against_production(
        data=data,
