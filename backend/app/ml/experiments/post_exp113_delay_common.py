...
def _experiment_aft_projects(frame,limit=None):
    ...
    work=frame[['canonical_project_id','snapshot_date','planned_completion_date']].copy()
    eligible=_p35.AFTResidualDelayModel._aft_eligible(work)
    return set(work.loc[eligible,'canonical_project_id'].astype('string').tolist())

@contextmanager
def _experiment_aft_compat():
    modules=(_p35,_p61,_pu1,_p105)
    previous=[getattr(m,'_select_aft_calibration_projects',None) for m in modules]
