...
def _experiment_aft_projects(frame,limit=None):
    required={'canonical_project_id','snapshot_date','planned_completion_date'};missing=sorted(required.difference(frame.columns))
    if missing: raise ValueError('Experiment AFT eligibility is missing required fields: '+', '.join(missing))
    work=frame[['canonical_project_id','snapshot_date','planned_completion_date']].copy();eligible=_p35.AFTResidualDelayModel._aft_eligible(work);return set(work.loc[eligible,'canonical_project_id'].astype('string').tolist())
@contextmanager
...
def _production_oof_fold(val,year,data=None,identity=None):
    ...
    with tempfile.TemporaryDirectory(prefix=f'prod-oof-{year}-') as td:
        root=Path(td)/'models'
        with _experiment_aft_compat(): train_current_production(2001,train_end,int(year),data=source_data,identity=source_identity,artifact_root=root)
