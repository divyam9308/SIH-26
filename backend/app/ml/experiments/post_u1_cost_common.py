"""Shared isolated Cost harness for Exp90-109.

Freshly retrains current production (Exp61 Cost + promoted U1 Delay), constructs
forward OOF predictions of the current Cost stack, and evaluates Cost-only
...
def _raw_oof(frame,features,family,max_folds=3):
    ...
    for fit,val,year in forward_folds(frame,max_folds):
        ...
        raw=np.asarray(model.predict(val.reindex(columns=features)),float);part=val.copy();part['prediction']=raw;part['residual']=pd.to_numeric(part['actual_cost_overrun_percentage'],errors='coerce').to_numpy(float)-raw;part['oof_year']=year;out.append(part)
...
def current_cost_oof(train,production_model):
    ...
    for fit,val,year in forward_folds(train,6):
        ...
        cal=shrunk_calibration(inner,40.0);model=_fit_pipeline(_regressors(PRODUCTION_COST_SEED)[family],fit,features,'actual_cost_overrun_percentage');raw=np.asarray(model.predict(val.reindex(columns=features)),float);pred=raw+_corrections(val,raw,cal)
...
def fit_residual_booster(oof,score,features,seed):
    ...
    scale=best[1];cols,med,xf,xs=numeric_design(oof,score,features);m=LGBMRegressor(n_estimators=140,learning_rate=.025,max_depth=3,num_leaves=8,min_child_samples=60,reg_alpha=4,reg_lambda=20,random_state=seed,verbosity=-1,n_jobs=1);r=pd.to_numeric(oof['residual'],errors='coerce').fillna(0).to_numpy(float);w=pd.to_numeric(oof['sample_weight'],errors='coerce').fillna(0).to_numpy(float);m.fit(xf,r,sample_weight=w);cap=max(_wq(np.abs(r),w,.9),1e-9);corr=scale*np.clip(np.asarray(m.predict(xs),float),-cap,cap)
