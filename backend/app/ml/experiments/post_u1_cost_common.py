"""Shared isolated Cost harness for post-PR110 experiments.

Freshly retrains current production, builds forward OOF predictions for the
current Exp12+Exp51 Cost path, and evaluates a Cost-only challenger on the exact
standard future cohort. Only 2001-2019 and 2001-2021 are supported.
"""
...
from backend.app.ml.experiments.nextgen_common import _family,_hash_prod,_prepare,normalize_taxonomy,shrunk_calibration
...
def _wq(v,w,q):
    ...
    o=np.argsort(v);v=v[o];w=w[o];s=float(w.sum())
    return float(np.quantile(v,q)) if s<=0 else float(v[np.searchsorted(np.cumsum(w),q*s,side='left')])

def _mae(y,p,w):
    y=np.asarray(y,float);p=np.asarray(p,float);w=np.asarray(w,float);m=np.isfinite(y)&np.isfinite(p)&np.isfinite(w)&(w>=0)
    return float(np.average(np.abs(y[m]-p[m]),weights=w[m])) if float(w[m].sum())>0 else float(np.mean(np.abs(y[m]-p[m])))
...
def _raw_oof(frame,features,family,max_folds=3):
    out=[]
    for fit,val,year in forward_folds(frame,max_folds):
        model=_fit_pipeline(_regressors(PRODUCTION_COST_SEED)[family],fit,features,'actual_cost_overrun_percentage')
        raw=np.asarray(model.predict(val.reindex(columns=features)),float);part=val.copy();part['prediction']=raw
        part['residual']=pd.to_numeric(part['actual_cost_overrun_percentage'],errors='coerce').to_numpy(float)-raw;part['oof_year']=year;out.append(part)
    return pd.concat(out,ignore_index=True) if out else pd.DataFrame()

def current_cost_oof(train,production_model):
    features=list(production_model.features);family=_family(production_model);out=[]
    for fit,val,year in forward_folds(train,6):
        inner=_raw_oof(fit,features,family,3)
        if inner.empty: continue
        cal=shrunk_calibration(inner,40.0);model=_fit_pipeline(_regressors(PRODUCTION_COST_SEED)[family],fit,features,'actual_cost_overrun_percentage')
        raw=np.asarray(model.predict(val.reindex(columns=features)),float);pred=raw+_corrections(val,raw,cal)
        part=val.copy();part['production_prediction']=pred;part['residual']=pd.to_numeric(part['actual_cost_overrun_percentage'],errors='coerce').to_numpy(float)-pred;part['oof_year']=year;out.append(part)
    if len(out)<2: raise ValueError('Need at least two forward current-production Cost OOF folds')
...
def numeric_design(train,score,features):
    ...
    for c in cols:
        ...
        a[c]=x.fillna(m);b[c]=y.fillna(m);med[c]=m
...
def fit_residual_booster(oof,score,features,seed):
    ...
    for year in years[1:]:
        ...
        cols,_,xf,xv=numeric_design(fit,val,features);m=LGBMRegressor(n_estimators=100,learning_rate=.025,max_depth=3,num_leaves=8,min_child_samples=60,reg_alpha=4,reg_lambda=20,random_state=seed,verbosity=-1,n_jobs=1)
        r=pd.to_numeric(fit['residual'],errors='coerce').fillna(0).to_numpy(float);w=pd.to_numeric(fit['sample_weight'],errors='coerce').fillna(0).to_numpy(float);m.fit(xf,r,sample_weight=w);cap=max(_wq(np.abs(r),w,.9),1e-9);meta.append((val,np.clip(np.asarray(m.predict(xv),float),-cap,cap)))
    if not meta: raise ValueError('No meta-OOF residual predictions')
    best=(float('inf'),0.0)
    for scale in (0.0,.25,.5,.75,1.0):
        ...
        s=float(np.average(vals,weights=weights))
        if (s,scale)<best: best=(s,scale)
    scale=best[1];cols,med,xf,xs=numeric_design(oof,score,features);m=LGBMRegressor(n_estimators=140,learning_rate=.025,max_depth=3,num_leaves=8,min_child_samples=60,reg_alpha=4,reg_lambda=20,random_state=seed,verbosity=-1,n_jobs=1)
    r=pd.to_numeric(oof['residual'],errors='coerce').fillna(0).to_numpy(float);w=pd.to_numeric(oof['sample_weight'],errors='coerce').fillna(0).to_numpy(float);m.fit(xf,r,sample_weight=w);cap=max(_wq(np.abs(r),w,.9),1e-9);corr=scale*np.clip(np.asarray(m.predict(xs),float),-cap,cap)
...
def persist(exp_id,name,ctx,experiment_cost,details,output):
    c=ctx['cohort'];pc=ctx['production_cost'];pdly=ctx['production_delay'];ec=np.asarray(experiment_cost,float);pcm=_metric(c,'actual_cost_overrun_percentage',pc);ecm=_metric(c,'actual_cost_overrun_percentage',ec);pdm=_metric(c,'actual_delay_days',pdly);cg=_gain(pcm,ecm);verdict='PROMOTION CANDIDATE' if cg>0 else 'DO NOT PROMOTE'
    result={'experiment_id':exp_id,'experiment_name':name,'scope':'cost','training_start':2001,'training_end':ctx['training_end'],'test_start':ctx['test_start'],'test_end':ctx['test_end'],'production_cost_mae':pcm,'experiment_cost_mae':ecm,'cost_improvement_percentage':round(cg,6),'production_delay_mae':pdm,'experiment_delay_mae':pdm,'delay_improvement_percentage':0.0,'comparison_test_projects':int(c['canonical_project_id'].nunique()),'comparison_test_snapshots':len(c),'delay_predictions_identical':True,'holdout_used_for_selection':False,'promotion_allowed':False,'execution_verdict':'EXECUTION VALID','scientific_verdict':verdict,'details':_json_safe(details)}
    p=Path(output);p.parent.mkdir(parents=True,exist_ok=True);p.write_text(json.dumps(_json_safe(result),indent=2,allow_nan=False)+'\n');mark=exp_id.replace('_','').upper();print(f'{mark}_2001_{ctx["training_end"]}_PRODUCTION_COST_MAE={pcm:.6f}');print(f'{mark}_2001_{ctx["training_end"]}_EXPERIMENT_COST_MAE={ecm:.6f}');print(f'{mark}_2001_{ctx["training_end"]}_COST_IMPROVEMENT_PERCENT={cg:.6f}');print(f'{mark}_2001_{ctx["training_end"]}_PRODUCTION_DELAY_MAE={pdm:.6f}');print(f'{mark}_2001_{ctx["training_end"]}_EXPERIMENT_DELAY_MAE={pdm:.6f}');print(f'{mark}_SCIENTIFIC_VERDICT={verdict}');return result
