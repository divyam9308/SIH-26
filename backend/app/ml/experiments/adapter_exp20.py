"""Retrain & Compare adapter for Experiment 20 LSTM forecasting."""
...
def fit_against_production(**kwargs):
    from backend.app.ml.experiments.lstm_sequence_exp20 import fit_experiment
...
def filter_comparable_rows(frame, state):
    from backend.app.ml.experiments.lstm_sequence_exp20 import filter_comparable_rows as implementation
    return implementation(frame, state)


def predict_project(row, state):
    from backend.app.ml.experiments.lstm_sequence_exp20 import predict_project as implementation
