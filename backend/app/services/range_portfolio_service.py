...
from backend.app.services.frozen_explanation_service import (
    _atomic_json,
    _metadata_path as explanation_metadata_path,
    _output_path as explanation_output_path,
...
def _saved_payload(window: str) -> dict | None:
    ...
    explanation_sha = saved.get("explanation_artifact_sha256")
    if explanation_sha:
        artifact = explanation_output_path(window)
        if not artifact.exists() or file_sha256(artifact) != explanation_sha:
            raise ValueError(f"Saved production view for {window} references a missing or stale explanation artifact: {path}")
        explanation_fields = {
            "cost_factors", "delay_factors", "risk_factors",
            "cost_explanation_status", "delay_explanation_status", "risk_explanation_status",
            "operational_drivers", "explanation_provenance",
        }
        incomplete = next(
            (item.get("project_code") for item in payload["items"] if explanation_fields - set(item)),
            None,
        )
...
def _signature(window: str) -> str:
    ...
    if manifest_path.exists() and ledger_path.exists():
        ...
        parts = ["frozen-ledger-v2", manifest["run_id"], manifest["dataset_fingerprint"], str(manifest_path.stat().st_mtime_ns), str(ledger_path.stat().st_mtime_ns), str(ledger_path.stat().st_size)]
        for path in (explanation_output_path(window), explanation_metadata_path(window)):
            if path.exists():
                stat = path.stat()
                parts.extend((path.name, str(stat.st_mtime_ns), str(stat.st_size)))
        return "|".join(parts)
    if window in CANONICAL_LIFECYCLE_ONLY_WINDOWS:
        _manifest(window)
...
def _payload(window: str, signature: str) -> dict:
    ...
    for item in items:
        explanation, status = verified_explanation(window, item["project_code"], item["snapshot_date"])
        if explanation:
            item.update({
                "cost_factors": explanation.get("cost", {}).get("factors", []),
                "delay_factors": explanation.get("delay", {}).get("factors", []),
                "risk_factors": explanation.get("risk", {}).get("factors", []),
                "cost_explanation_status": {key: explanation.get("cost", {}).get(key) for key in ("available", "reason", "source")},
                "delay_explanation_status": {key: explanation.get("delay", {}).get(key) for key in ("available", "reason", "source")},
                "risk_explanation_status": {key: explanation.get("risk", {}).get(key) for key in ("available", "reason", "source")},
                "operational_drivers": explanation.get("operational_drivers", []),
                "explanation_provenance": {
                    "run_id": explanation.get("run_id"),
                    "dataset_fingerprint": explanation.get("dataset_fingerprint"),
                    "cache_identity": explanation.get("cache_identity"),
            ...
            item.update({
                "cost_factors": [], "delay_factors": [], "risk_factors": [],
                "cost_explanation_status": status, "delay_explanation_status": status, "risk_explanation_status": status,
                "operational_drivers": [], "explanation_provenance": None,
            })
...
def write_saved_window_view(window: str) -> Path:
    ...
    document = {
        ...
        "explanation_artifact_sha256": file_sha256(explanation_output_path(window)) if explanation_output_path(window).exists() else None,
        "payload": payload,
    }
...
def historical_project(code: str, window: str) -> dict:
    ...
    unavailable_status = {"available": False, "reason": "No published explanation exists in this saved project view.", "source": None}
    cost_factors = item.get("cost_factors", [])
